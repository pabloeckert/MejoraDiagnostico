// forked from design-sync lib/source-kit.mjs — MejoraDiagnostico's components
// are all `export default function Name()`; `export *` never re-exports a
// default, so the synth entry needs an explicit `export { default as Name }`
// alias per file or window.<globalName>.<Name> never gets assigned.
//
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    // `export *` never re-exports a default — alias it under its declared
    // name so `export default function Name()` files still reach the bundle.
    ...
    for (const p of comps) {
      ...
      const defaultName = sf
        ?.getExportedDeclarations()
        .get('default')
        ?.map((d) => d.getName?.())
        .find((n) => n && n !== 'default');
      if (defaultName && /^[A-Z][A-Za-z0-9]*$/.test(defaultName)) {
        lines.push(`export { default as ${defaultName} } from ${JSON.stringify(p)};`);
      }
    }
